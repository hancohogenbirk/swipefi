...
- Reorderable queue with drag-to-reorder, skip-to, and sort-aware metadata column (play count or date added) with consistent icon and accent color styling
- Queue context display showing source folder and sort order in the queue header
- Swipe gestures in queue: swipe left to reject (delete), swipe right to remove from queue
...
- Transcode detection — flags FLAC files transcoded from lossy sources (powered by [flacalyzer](https://github.com/hancohogenbirk/flacalyzer), warning badge on album art, toggle in Settings, runs on rescan)
- Spotify-style full-width progress bar with elapsed/remaining times below
- Smart ±15s skip — taps coalesce within a 300 ms window (5 fast taps = one +75 s seek, not 5 separate seeks); advances to the next track when the cumulative jump lands past end
